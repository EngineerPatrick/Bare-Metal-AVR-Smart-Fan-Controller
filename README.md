...
The firmware has an initial configuration phase where the user input a temp/speed fan-curve (10 nodes) using a display,
3 push buttons and a rotary encoder. Then, the firmware advances to a runtime loop where it measures temperature from a sensor,
calculates the target speed using linear interpolation, drives a 4-wire PWM fan and displays the measured temp/speed values.

## Features

- Resource-constrained firmware
- Thread-safe interrupts
- Scheduler service for real-time coordination
- Aggressive error checks with error propagation
- TWI/I2C driver with finite-state machine
...
The fan-curve can be configured only with constant or increasing temperature and speed values within the valid range.
...
5. The next node has the values of the preceding one and can be set only with equal or higher values
6. If the "select" button is pressed when the last node is selected, the UI moves back to the first node
	- In this case, if one node is modified with higher values they get copied to all the successive nodes with lower values
...
- If an error with the display occurs LED1 lights up
- If an error with the EEPROM occurs LED2 lights up and the display shows "Err 2"
- If an error with the BME280 occurs LED3 lights up and the display shows "Err 3"
- If an error with the fan occurs the buzzer emits a sound and the display shows "Err 4"
