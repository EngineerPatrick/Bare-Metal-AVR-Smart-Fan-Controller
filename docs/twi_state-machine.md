...
- Uses a non-blocking ISR with simple logic
...
The ISR is only responsible to check for possible errors caused by the previous operation, call `twi_state_advance` and perform the next
...
4. The hardware timeout is employed to ensure that the STOP condition is sent and the TWI hardware is liberated within 400 ms
