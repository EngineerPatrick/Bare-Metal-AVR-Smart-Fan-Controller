...
*
*	The curve is saved in the EEPROM and validated with CRC to restrain
...
*	@retval	FAN_CURVE_ERR_EEPROM 	If the curve is not correctly saved in the EEPROM
...
*
*	@pre 	0 < curve_size <= 10
*	@pre 	node_temp points to the first element of an array containing the temperatures of all nodes
*	@pre 	node_speed points to the first element of an array containing the speeds of all nodes
*	@pre 	Passed temperatures or speeds are within valid boundaries and are increasingly higher or constant
*   @post   If invalid parameters are passed the curve saved in the EEPROM is left unchanged
*   @post   If the curve is not correctly saved in the EEPROM changes to the curve saved in the EEPROM are left
*	@post 	On success the curve saved in the EEPROM is updated with the new size, temperature and speed values
*
*	@par 	Invariants:
*
*			-The size of the curve saved in the EEPROM is always 10
*
*			-The number of updated nodes matches curve_size and is always bigger than 0
*
*			-The sequence of the nodes matches that of the temperature array and the speed array
*
...
/**
*
*	@brief	Calculates the speed value for the current temperature from the curve saved in the EEPROM.
*
*	@param	temp_c					Measured current temperature
*	@param	speed_rpm				Pointer to the variable where to save the speed value
*
*	@retval	FAN_CURVE_ERR_OK 		If no error occurs
*	@retval	FAN_CURVE_ERR_EEPROM 	If the curve is not correctly loaded from the EEPROM or if fan_curve_create has never been called
...
*   @post   If the curve is not correctly loaded from the EEPROM or fan_curve_create has never been called speed_rpm is left unchanged
*	@post 	On success if temp_c < lowest temperature the speed value matches the lowest speed
*	@post 	On success if temp_c > highest temperature the speed value matches the highest speed
*	@post 	On success if temp_c falls between two nodes a linear interpolation is performed to evaluate the speed value
*	@post 	On success the speed value from the curve is saved in *speed_rpm
*
*	@par 	Invariants:
*
*			-The number of nodes of the curve saved in the EEPROM used to calculate the speed value matches the curve_size field of the
*			curve saved in the EEPROM
*
*/
